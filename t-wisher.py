...
def check_terms():
    ...
    choice = input(Fore.CYAN + "[?] Do you belong to authorized category? (yes/no): " + Style.RESET_ALL).lower()
    ...
    cat_choice = input(Fore.CYAN + "[?] Enter category (1-5): " + Style.RESET_ALL)
...
def get_port():
    ...
    while True:
        try:
            port = input(Fore.CYAN + "\n[?] Enter port for hunt (default: 8080): " + Style.RESET_ALL)
            if not port:
                return 8080
            port = int(port)
            if 1 <= port <= 65535:
...
def select_tunnel():
    ...
    while True:
        choice = input(Fore.CYAN + "[?] Enter choice (1-5): " + Style.RESET_ALL)
...
def start_server(port):
    ...
    if tunnel_method == '2':
        ...
        if public_url:
            ...
            qr_file = generate_qr(public_url, 'ngrok')
            if qr_file:
                print(Fore.GREEN + f"[+] QR Code saved: {qr_file}")
        ...
        if public_url:
            ...
            qr_file = generate_qr(public_url, 'serveo')
            if qr_file:
                print(Fore.GREEN + f"[+] QR Code saved: {qr_file}")
        ...
        if public_url:
            ...
            qr_file = generate_qr(public_url, 'cloudflared')
            if qr_file:
                print(Fore.GREEN + f"[+] QR Code saved: {qr_file}")
        ...
        if public_url:
            ...
            qr_file = generate_qr(public_url, 'sshlocal')
            if qr_file:
                print(Fore.GREEN + f"[+] QR Code saved: {qr_file}")
